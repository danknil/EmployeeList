...
import employee.database.service.base.ElectroEmployeeLocalServiceBaseImpl;
		...
		try (Scanner scanner = new Scanner(inputStream)) {
			while (scanner.hasNextLine())
				try(Scanner lineScanner = new Scanner(scanner.nextLine())) {
					...
					ElectroEmployee ee = createElectroEmployee(lineScanner.nextLong());
					ee.setEmployeeId(lineScanner.nextLong());

					addElectroEmployee(ee);
				}
		}
	}
}